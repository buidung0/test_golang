...
func RenderTemplate(w http.ResponseWriter, tmpl string, td *models.TemplateData) {
	...
	t, ok := tc[tmpl]
	if !ok {
		log.Fatal("Could not get template from teemplate cache")
	}
	...
	parseTemlate, _ := template.ParseFiles("./templates/" + tmpl)
	err = parseTemlate.Execute(w, nil)
	if err != nil {
...
//CreateTemplateTest creates a template cache as a map
func CreateTemplateCache() (map[string]*template.Template, error) {
	...
	for _, page := range page {
		...
		// fmt.Println("Page is currentcy", page)
